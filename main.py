...
class GamesApp:
    def __init__(self, page: ft.Page):
        ...
        self.developer_site_url = "http://localhost:8000"
        ...
        self.combo_filter_col = ft.Dropdown(
            label="Фильтр по:",
            options=[ft.dropdown.Option("(Нет)")]
            + [ft.dropdown.Option(c) for c in self.display_cols],
            value="(Нет)",
            expand=1,
            border_color="#2A2F42",
        ...
        self.entry_cols = ft.TextField(
            label="Столбцы (id, name, price, category...)",
            expand=1,
            border_color="#2A2F42",
            focused_border_color="#6366F1",
        )
        ...
        self.entry_add_name = ft.TextField(
            label="Название игры",
            expand=2,
            border_color="#2A2F42",
            focused_border_color="#6366F1",
        ...
        self.entry_add_category = ft.TextField(
            label="Жанр",
            expand=1,
            border_color="#2A2F42",
            focused_border_color="#6366F1",
        )
        self.entry_add_price = ft.TextField(
            label="Цена",
            expand=1,
            border_color="#2A2F42",
            focused_border_color="#6366F1",
        )
        self.entry_add_developer = ft.TextField(
            label="Разработчик",
            expand=1,
            border_color="#2A2F42",
            focused_border_color="#6366F1",
        )
        self.entry_add_release_date = ft.TextField(
            label="Дата выхода",
            expand=1,
            border_color="#2A2F42",
            focused_border_color="#6366F1",
        )
        self.entry_add_rating = ft.TextField(
            label="Рейтинг",
            expand=1,
            border_color="#2A2F42",
            focused_border_color="#6366F1",
        )
        self.entry_add_discount = ft.TextField(
        ...
        self.entry_delete_id = ft.TextField(
            label="ID для удаления",
            expand=1,
            border_color="#2A2F42",
    ...
    def build(self):
        self.page.appbar = ft.AppBar(
            title=ft.Text(
                "Магазин Игр ZPay", size=18, weight=ft.FontWeight.W_600, color="#F8FAFC"
            ),
            bgcolor="#161926",
            center_title=False,
            actions=[
        ...
        filter_card = ft.Container(
            content=ft.Column(
                [
                    ...
                    ft.Row(
                        [self.switch_asc, self.btn_apply],
        ...
        management_card = ft.Container(
            content=ft.Column(
                [
                    ft.Text(
                        "Управление базой данных (Добавление / Удаление)",
                        color="#F43F5E",
                        weight=ft.FontWeight.BOLD,
                        size=14,
                    ),
                    ft.Row(
                        [
                            self.entry_add_name,
                            self.entry_add_category,
                            self.entry_add_price,
                        ],
                        spacing=10,
                    ),
                    ft.Row(
                        [
                            self.entry_add_developer,
                            self.entry_add_release_date,
                            self.entry_add_rating,
                            self.entry_add_discount,
                        ],
                        spacing=10,
                    ),
        ...
        status_bar = ft.Container(
            content=self.status_text, padding=ft.Padding.only(left=5, top=5, bottom=5)
        )

        return ft.Container(
            content=ft.Column(
                [filter_card, management_card, table_container, status_bar],
                spacing=15,
                scroll=ft.ScrollMode.ADAPTIVE,
            ),
            padding=10,
        ...
        try:
            ...
            if "1." in report_name:
                ...
                df_abc = pd.read_sql_query(
                    "SELECT name as 'Название', price as 'Выручка' FROM games", conn
                )
                total_rev = df_abc["Выручка"].sum()
                df_abc = df_abc.sort_values(by="Выручка", ascending=False)
                df_abc["Доля (%)"] = (
                    (df_abc["Выручка"] / (total_rev if total_rev > 0 else 1)) * 100
                ).round(2)
    ...
    def update_table(self):
        ...
        if self.current_df.empty:
            self.data_table.columns.append(
                ft.DataColumn(ft.Text("Нет данных для отображения", color="#94A3B8"))
            )
        ...
        for col in self.current_df.columns:
            ...
            self.data_table.columns.append(
                ft.DataColumn(
                    ft.Text(
                        str(header_text), weight=ft.FontWeight.W_600, color="#F8FAFC"
                    )
                )
            )
        ...
        for row in self.current_df.itertuples(index=False, name=None):
            cells = [
                ft.DataCell(
                    ft.Text(str(val) if pd.notna(val) else "-", color="#E2E8F0")
                )
                for val in row
            ]
    ...
    def apply_ops(self, e=None):
        ...
        f_val = (
            self.entry_filter_val.value.strip() if self.entry_filter_val.value else ""
        )
        ...
        if f_col_display != "(Нет)" and f_val:
            ...
            if actual_col in df.columns:
                df = df[
                    df[actual_col].astype(str).str.contains(f_val, case=False, na=False)
                ]
    ...
    def export_txt(self, e=None):
        ...
        try:
            ...
            self._show_snackbar(
                f"Сохранено в файл {os.path.basename(self.report_txt)}!"
            )
        except Exception as ex:
    ...
    def export_excel(self, e=None):
        ...
        try:
            ...
            self._show_snackbar(
                f"Сохранено в файл {os.path.basename(self.report_excel)}!"
            )
        except Exception as ex:
            self._show_snackbar(f"Ошибка экспорта: {str(ex)}", error=True)
